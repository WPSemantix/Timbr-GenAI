...
-- order_detail table load - fails due to large size, loading a sample from a different script file
--COPY INTO tb_101.raw_pos.order_detail
--FROM @tb_101.public.s3load/raw_pos/order_detail/;


-- LIST @tb_101.public.s3load/raw_pos/order_detail/ PATTERN='.*order_detail_[0-3]_[0-9]+_[0-9]+\\.csv\\.gz';
COPY INTO tb_101.raw_pos.order_detail
FROM @tb_101.public.s3load/raw_pos/order_detail/
FILE_FORMAT = (TYPE = 'CSV' COMPRESSION = 'GZIP')
PATTERN = '.*order_detail_[0-3]_[0-9]+_[0-9]+\\.csv\\.gz';

ALTER WAREHOUSE tb_de_wh SET WAREHOUSE_SIZE = 'XSmall';

-- setup completion note
SELECT 'tb_101 setup is now complete' AS note;TB_101.PUBLIC.S3LOAD
